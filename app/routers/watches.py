...
async def list_watches(
    ...
    collection: Optional[str] = Query(None, description="Filter by collection name (case-insensitive)"),
    min_price: Optional[float] = Query(None, ge=0, description="Minimum retail price filter"),
    max_price: Optional[float] = Query(None, ge=0, description="Maximum retail price filter"),
    min_size: Optional[int] = Query(None, ge=0, description="Minimum watch case size (mm)"),
    max_size: Optional[int] = Query(None, ge=0, description="Maximum watch case size (mm)"),
    complication: Optional[str] = Query(None, description="Filter by watch complication"),
    is_por: Optional[bool] = Query(None, description="Filter by Price On Request status"),
    sort_by: str = Query("reference", pattern="^(reference|collection|price|size)$", description="Field to sort by"),
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    responses={
        status.HTTP_404_NOT_FOUND: {"model": ErrorResponse, "description": "Watch reference not found"}
    },
...
async def get_watch_by_reference(
    reference: str = Path(..., description="Rolex watch reference number (e.g., '116500LN')"),
